...
export default function Home() {
  ...
  }, [loadMatchHistory, loadTournamentHistory])
  ...
  }, [playerNames, loadMatchHistory, loadTournamentHistory])
  ...
  return (
    <div className="container">
      ...
      <div id="tournament-history" className="bg-white/10 p-6 rounded-xl my-5 text-left">
        <h3 className="text-xl font-semibold mb-4 text-white">Архив турниров</h3>
        <div className="flex flex-col sm:flex-row gap-3 mb-4 flex-wrap">
          <Button
        ...
        <ul id="tournament-history-list" className="list-none max-h-[200px] overflow-y-auto pr-2">
          {isLoadingTournamentHistory ? (
            <li className="text-white/60 italic text-sm">Загрузка истории турниров...</li>
          ) : tournamentHistory.length === 0 ? (
            <li className="text-white/60 italic text-sm">История турниров пуста</li>
          ) : (
            tournamentHistory.map((tournament, index) => (
              <li
                key={tournament.id || index}
                className="bg-white/15 p-3 my-1 rounded-md border-l-4 border-yellow-500 text-sm transition-all duration-300 ease-in-out hover:bg-white/20 hover:translate-x-1 hover:border-green-400 hover:shadow-md cursor-pointer"
              >
                {formatTournamentEntry(tournament)}
              </li>
            ))
          )}
        </ul>
